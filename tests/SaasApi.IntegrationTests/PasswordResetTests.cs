...
public class PasswordResetTests(WebAppFactory factory) : IntegrationTestBase(factory)
{
    ...
    {
        await Client.PostAsJsonAsync("/api/tenants", new { name = "Reset Co 1", slug = "reset-co-1" });
    ...
    {
        await Client.PostAsJsonAsync("/api/tenants", new { name = "Reset Co 2", slug = "reset-co-2" });
    ...
    {
        await Client.PostAsJsonAsync("/api/tenants", new { name = "Reset Co 3", slug = "reset-co-3" });
    ...
    {
        await Client.PostAsJsonAsync("/api/tenants", new { name = "Reset Co 5", slug = "reset-co-5" });
        ...
        // Use the token once
        await Client.PostAsJsonAsync("/api/auth/reset-password", new { token, newPassword = "NewPass1!" });

        // Using it again should return 404
        var replayResponse = await Client.PostAsJsonAsync("/api/auth/reset-password", new
        {
            token,
