...
public class ProductTests(WebAppFactory factory) : IntegrationTestBase(factory)
{
    ...
    public async Task CreateProduct_Returns201()
    {
        ...
        var tenantResponse = await Client.PostAsJsonAsync("/api/v1/tenants", new { name = "Test Co", slug = "testco-prod" });
    ...
    {
        var tenantResponse = await Client.PostAsJsonAsync("/api/v1/tenants", new { name = "Test Co 2", slug = "testco-prod2" });
        var tenant = await tenantResponse.Content.ReadFromJsonAsync<TenantResult>();
